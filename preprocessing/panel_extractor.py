# This module is taken from https://github.com/mstyczen/comic-dcp, credits to @mstyczen
...
class PanelExtractor:
    def extract_and_save_panels(
        self, img_paths, out_directory, iterations=15, thresh_type="adaptive"
    ):
        """
        ...
        out_paths = []
        for path in tqdm(img_paths, desc="Panel Extraction - Completed Images"):
            img = cv2.imread(path)
    ...
    def extract_panels(self, image_path, iterations, thresh_type):
        ...
        opening = cv2.morphologyEx(
            opening, cv2.MORPH_OPEN, kernel, iterations=iterations
        )
        ...
        bboxes = sorted(
            ...
            key=lambda bbox: 5 * bbox[1] + bbox[0],
        )
    ...
    def visualize(image, gray, thresh, contours_img, opening, bboxes):
        ...
        for x, y, w, h in bboxes:
            bboxes_image = cv2.rectangle(
                bboxes_image, (x, y), (x + w, y + h), (36, 255, 12), 3
            )

        images = np.hstack(
            (gray_rgb, thresh_rgb, contours_img, opening_rgb, bboxes_image)
        )
